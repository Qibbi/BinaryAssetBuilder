...
namespace BinaryAssetBuilder.Core
{
    public sealed class BinaryAsset : IDisposable
    {
        ...
        private void UpdateMemoryAvailability(bool forceUpdate)
        {
            ...
            _memoryAvailability = _buffer is not null && _buffer.InstanceData is not null && _buffer.RelocationData is not null && _buffer.ImportsData is not null
        ...
        private void UpdateLocalAvailability(bool forceUpdate)
        {
            ...
            _localAvailability = _lastLocationInfo is not null ? Availability.Present : Availability.Missing;
        }
        ...
        private void UpdateBasePatchStreamAvailability(bool forceUpdate)
        {
            ...
            _basePatchStreamAvailability = _parent.BasePatchStreamAssets is not null && _parent.BasePatchStreamAssets.TryGetValue(FileBase, out _assetHeader)
                ? Availability.Present
                : Availability.Missing;
        }
        ...
        {
            if (_assetHeader is not null && !forceUpdate)
            {
                return;
            }
        ...
        {
            if (_assetHeader is not null || GetLocation(AssetLocation.All, AssetLocationOption.None) == AssetLocation.None)
            {
                return;
            }
